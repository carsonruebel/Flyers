...
def pull_data(storename):
    ...
    if exists('adhistory.xlsx') == 0:
        ...
        with pd.ExcelWriter('adhistory.xlsx') as writer:  
            df_new.to_excel(writer, sheet_name='RawInput', index=False)
...
def merge_data(filename, dataframe):
    ...
    with pd.ExcelWriter('adhistory.xlsx', mode='a', if_sheet_exists='replace') as writer:  
        df_final.to_excel(writer, sheet_name='RawInput', index=False)
